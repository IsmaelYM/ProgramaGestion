...
public class EliminarProducto implements WindowListener, ActionListener{
	Frame ventana = new Frame("Baja Producto");
	Dialog dlgSeguro = new Dialog(ventana, "¿Segur@?", true);
	Dialog dlgMensaje = new Dialog(ventana, "Mensaje", true);
	
	Label lblElegir = new Label("Elegir el producto a eliminar:");
	Label lblSeguro = new Label("¿Seguro de eliminar ");
	Label lblMensaje = new Label("Producto Eliminado");
	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource().equals(btnEliminar))
		{
			if(choProductos.getSelectedIndex()!=0)
			{
				...
				lblSeguro.setText("¿Seguro de eliminar "+choProductos.getSelectedItem()+"?");
		...
		else if(e.getSource().equals(btnSi))
		{
			...
			int respuesta = conexion.eliminarProducto(tabla[0]);
